...
public class MeteringTypeResourceImpl {
	...
	public Response getAll(
		...
		List<MeteringTypeDto> list = service.find(null, null, name, buildSessionContext(lang))
			.stream()
			.map( it-> mapper.getMapper().map(it, MeteringTypeDto.class) )
	...
	public Response getById(@PathParam("id") Long id, @QueryParam("lang") Lang lang) {
		...
		return Response.ok()
			.entity(mapper.getMapper().map(meteringType, MeteringTypeDto.class))
			.build();		
	}
	...
	public Response create(MeteringTypeDto entityDto) {
		MeteringType entity = mapper.getMapper().map(entityDto, MeteringType.class);
		MeteringType newMeteringType = service.create(entity, buildSessionContext(entityDto.getLang()));

		return Response.ok()
			.entity(mapper.getMapper().map(newMeteringType, MeteringTypeDto.class))
			.build();
	}
